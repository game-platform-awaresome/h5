...
//        $signon_people = $m_user->fetch("tg_channel ={$tg_channel}", 'COUNT(user_id) AS reg_people');
        ...
        $peopel_stat=$m_sign->fetchAllBySql("select game_id, COUNT(*) AS signon_times, COUNT(DISTINCT h5.signon_log.user_id) AS signon_people  from h5.signon_log  inner join h5.`user`  on h5.signon_log.user_id = h5.`user`.user_id where h5.`user`.tg_channel =  {$tg_channel}");
        if($peopel_stat){
            $conds='';
