...
class Game_Pager
{
	...
	public function __construct($total, $url = null, $marker = 'pn', $half = 5/*, $perpage = null*/)
	{
		...
		if( $current > 1 ) {
			...
			$this->_pagelink .= '<li class="prev"><a href="'.str_replace($marker,$current-1,$url).'">&lt;</a></li>';
		} else {
		    $this->_pagelink .= '<li class="prev"><a>&lt;</a></li>';
		}
		
		for($i = $current - $half, $i = ($i > 0) ? $i : 1, $j = $current + $half, $j = ($j > $total) ? $total : $j; $i <= $j; $i++)
		{
			( $i == $current ) ? 
				$this->_pagelink .= '<li class="page active"><a>'.$i.'</a></li>' : 
				$this->_pagelink .= '<li class="page"><a href="'.str_replace($marker,$i,$url).'">'.$i.'</a></li>';
		}
		
		if( $current < $total ) {
			$this->_pagelink .= '<li class="next"><a href="'.str_replace($marker,$current+1,$url).'">&gt;</a></li>';
			//$this->_pagelink .= '<a href="'.str_replace($marker,$total,$url).'">尾页</a>';
		} else {
		    $this->_pagelink .= '<li class="next"><a>&gt;</a></li>';
		}
		/*
		if( $perpage ) {
