...
class usermodel {
	...
	function check_username($username) {
		$guestexp = '\xA1\xA1|\xAC\xA3|^Guest|^\xD3\xCE\xBF\xCD|\xB9\x43\xAB\xC8|admin|game|sys|^gm|root|管理员';
		if( preg_match("/\s+|^c:\\con\\con|[%,\*\"\s\<\>\&]|$guestexp/is", $username) ) {
		    return FALSE;
		}
		$len = $this->dstrlen($username);
		if( $len == 11 && preg_match('/^(?:13[0-9]|14[57]|15[0-9]|17[0678]|18[0-9])\d{8}$/', $username) ) {
		    return TRUE;
		}
		if( strpos($username, '@') && $len < 33 && $this->check_emailformat($username) ) {
		    return TRUE;
		}
		if($len < 33 && $len > 3) {
		    return TRUE;
