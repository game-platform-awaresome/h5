...
//            $this->forward('notify', 'pigpay',$params);
    ...
    public function serverListAction(){
        ...
        if( $servers['start'] == '' ) {
            ...
            $servers_list = $m_server->fetchAllBySql("select h5.game.*,h5.server.*,h5.server.name as server_name,h5.server.add_time as server_add_time from h5.server left join h5.game on h5.game.game_id=h5.server.game_id where {$condition} order by {$order}");
