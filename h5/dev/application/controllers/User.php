...
//	    }
	...
	public function wxloginAction()
	{
        ...
        if($this->isMobile()){
            ...
            //Pc
            $url = "https://open.weixin.qq.com/connect/qrconnect?appid={$appid}&redirect_uri={$callback}&response_type=code&scope=snsapi_login&state={$state}#wechat_redirect";
        }
	    $this->redirect($url);
	    return false;
	}
...
//        $logs = $m_user->giftLogs($user['user_id'], $pn, $limit);
        $logs=$m_user->fetchAll(['player_channel'=>$_SESSION['user_id']],$pn,$limit,'username,reg_time,player_channel_get','reg_time desc');
        $this->getView()->assign('logs', $logs);
    }
