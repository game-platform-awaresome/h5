...
//    }
    ...
    public function getComment($game_id, $pn = 1, $limit = 6)
    {
        ...
        foreach ($logs as $key =>&$value){
            $value['children']=$this->fetchAllBySql("SELECT comment.*,user.username,user.avatar FROM comment LEFT join user on user.user_id=comment.user_id where parent_id={$value['comm_id']} ORDER BY comm_time desc");
            //回复人的id
        }
        return $logs;
    }
